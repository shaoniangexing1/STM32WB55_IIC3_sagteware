...
/** @addtogroup STM32WBxx_System_Private_Variables
  ...
  */
  ...
  const uint32_t MSIRangeTable[12] = {100000UL,   200000UL,   400000UL,   800000UL,  1000000UL,  2000000UL, \
...
/**
  ...
  *             in voltage and temperature.
  *
  *         (**) HSI_VALUE is a constant defined in stm32wbxx_hal.h file (default value
  *              16 MHz) but the real value may vary depending on the variations
  *              in voltage and temperature.
  *
  *         (***) HSE_VALUE is a constant defined in stm32wbxx_hal.h file (default value
...
void SystemCoreClockUpdate(void)
{
  ...
  if((RCC->CR & RCC_CR_MSIRGSEL) == 0UL)
  { /* MSISRANGE from RCC_CSR applies */
    msirange = (RCC->CSR & RCC_CSR_MSISRANGE) >> 8UL;
  }
  else
  { /* MSIRANGE from RCC_CR applies */
    msirange = (RCC->CR & RCC_CR_MSIRANGE) >> 4UL;
  }
  ...
  {
    case 0x00UL:  /* MSI used as system clock source */
      SystemCoreClock = msirange;
      break;

    case 0x04UL:  /* HSI used as system clock source */
      SystemCoreClock = HSI_VALUE;
      break;

    case 0x08UL:  /* HSE used as system clock source */
      SystemCoreClock = HSE_VALUE;
      break;

    case 0x0CUL:  /* PLL used as system clock  source */
      /* PLL_VCO = (HSE_VALUE or HSI_VALUE or MSI_VALUE/ PLLM) * PLLN
         SYSCLK = PLL_VCO / PLLR
         */
      pllsource = (RCC->PLLCFGR & RCC_PLLCFGR_PLLSRC);
      pllm = ((RCC->PLLCFGR & RCC_PLLCFGR_PLLM) >> 4UL) + 1UL ;

      switch (pllsource)
      {
        case 0x02UL:  /* HSI used as PLL clock source */
      ...
      pllvco = pllvco * ((RCC->PLLCFGR & RCC_PLLCFGR_PLLN) >> 8UL);
      pllr = (((RCC->PLLCFGR & RCC_PLLCFGR_PLLR) >> 25UL) + 1UL) * 2UL;
  ...
  /* Get HCLK prescaler */
  tmp = AHBPrescTable[((RCC->CFGR & RCC_CFGR_HPRE) >> 4UL)];
