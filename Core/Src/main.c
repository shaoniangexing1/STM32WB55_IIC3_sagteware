...
/* USER CODE BEGIN PV */
static void EnterStop2_ForSeconds(uint32_t seconds);
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
   ...
   // ✅ 添加 RTC WakeUp 中断 NVIC 配置 - 解决睡眠不唤醒问题
  ...
  while (1)
  {
    IIC3_Transmit(0xA0, (uint8_t *)"hello", 5, 1000);
    // 改为进入 STOP2 模式：1 秒深度睡眠
    EnterStop2_ForSeconds(1);
    /* USER CODE END WHILE */
...
extern volatile uint8_t g_rtc_wakeup_flag; // 外部声明唤醒标志
...
static void EnterSleepForSeconds(uint32_t seconds)
{
    ...
    const uint32_t wake_clk = RTC_WAKEUPCLOCK_CK_SPRE_16BITS; // 1Hz
...
#endif
    if(seconds > 0xFFFF) seconds = 0xFFFF;
    if(HAL_RTCEx_SetWakeUpTimer_IT(&hrtc, seconds-1, wake_clk) != HAL_OK){
        HAL_ResumeTick();
        return; // WakeUp 设置失败
    }
...
static void EnterStop2_ForSeconds(uint32_t seconds)
{
    ...
    const uint32_t wake_clk = RTC_WAKEUPCLOCK_CK_SPRE_16BITS; // 1Hz
#elif defined(RTC_WAKEUPCLOCK_CK_SPRE)
    const uint32_t wake_clk = RTC_WAKEUPCLOCK_CK_SPRE;        // 1Hz
#else
#error "No 1Hz wakeup clock macro defined for this HAL."
#endif
    if(seconds > 0xFFFF) seconds = 0xFFFF;
    if(HAL_RTCEx_SetWakeUpTimer_IT(&hrtc, seconds-1, wake_clk) != HAL_OK){
        HAL_ResumeTick();
        return; // WakeUp 设置失败，不进入低功耗
    }
    
    g_rtc_wakeup_flag = 0; // 清除唤醒标志
    __SEV(); __WFE(); __WFE(); // 清除挂起事件
    
    // 使用循环确保只被 RTC 唤醒，避免被其他中断误唤醒
    while(!g_rtc_wakeup_flag) {
        // 使用 HAL 库函数进入 STOP2 模式 (会自动设置 SLEEPDEEP)
        HAL_PWREx_EnterSTOP2Mode(PWR_STOPENTRY_WFI);
        // 如果被其他中断唤醒会继续循环，直到 RTC 标志置位
    }
    
    // ===== 从 STOP2 唤醒后执行 =====
#if defined(PWR_FLAG_STOPF)
    __HAL_PWR_CLEAR_FLAG(PWR_FLAG_STOPF); // 清除 STOP 标志
#endif
    HAL_RTCEx_DeactivateWakeUpTimer(&hrtc);
    SystemClock_Config(); // STOP2 会关闭主时钟，需重新配置
    HAL_ResumeTick();
}
/* USER CODE END 4 */

/* ✅ RTC WakeUp 中断处理 - 解决睡眠不唤醒问题 */
volatile uint8_t g_rtc_wakeup_flag = 0; // 全局唤醒标志
